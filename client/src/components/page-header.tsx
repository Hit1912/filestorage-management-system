        ...
        <div className="w-full flex flex-col gap-3 items-start justify-start lg:items-center lg:flex-row lg:justify-between">
          {(title || subtitle) && (
            <div className="space-y-1.5">
              {title && (
                <h1 className="break-normal font-bold text-xl sm:text-3xl tracking-tight text-foreground/95">
                  {title}
                </h1>
              )}
              {subtitle && (
                <p className="text-muted-foreground/80 text-xs sm:text-sm font-medium">
                  {subtitle}
                </p>
              )}
            </div>
