          ...
          {Object.entries(CHART_TYPES).map(([key, config]) => {
            ...
            return (
              <button
                ...
                className="relative z-30 flex flex-1 flex-col justify-center gap-1 border-t sm:border-l px-4 py-3 text-left transition-colors even:border-l data-[active=true]:bg-muted/50 sm:border-t-0 sm:px-8 sm:py-6 !cursor-pointer space-y-1"
                onClick={() => {
                  if (isLoading) return;
              ...
              >
                {isLoading ? (
                  <div className="space-y-1 min-w-32 min-h-20 sm:min-w-48 sm:min-h-28">
                    <Skeleton className="h-4 sm:h-6 w-24 sm:w-48" />
                  ...
                  <>
                    ...
                    <span className="hidden sm:flex min-w-[17ch] gap-x-1 text-xs text-muted-foreground">
                      +{value} past 30 days
                    </span>
                  </>
                )}
