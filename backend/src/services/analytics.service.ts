...
export const getUserAnalyticsWithChartService = async (
  ...
  if (Env.MOCK_MODE === 'true') {
    const userStorage = mockDb.storage.find((s: any) => s.userId === userId) || { totalSize: 0 };
    return {
      chart: [],
      totaluploadFilesForPeriod: mockDb.files.filter((f: any) => f.userId === userId).length,
      totalUsageForPeriod: formatBytes(userStorage.totalSize),
      storageUsageSummary: {
        totalUsage: userStorage.totalSize,
        quota: 10 * 1024 * 1024 * 1024, // 10 GB
        formattedTotalUsage: formatBytes(userStorage.totalSize),
        formattedQuota: formatBytes(10 * 1024 * 1024 * 1024),
      },
    };
  }
